export const validateName = (value: string) => validateCredentials({
  minLength: {
    condition: 2,
...
export const validateMiddleName = (value: string) => validateCredentials({
  minLength: {
    condition: 2,
    message: 'Минимальная длина - 2 символа'
  },
...
export const validateLastName = (value: string) => validateCredentials({
  minLength: {
    condition: 2,
    message: 'Минимальная длина - 2 символа'
  },
...
export const validateFullName = (value: string) => validateCredentials({
  minLength: {
    condition: 2,
    message: 'Минимальная длина - 2 символа'
  },
...
export const validateTextarea = (value: string) => {
  ...
  const pattern = {
    ...
    message: 'Поле может содержать русские/латинские буквы и символы "-.,?!-:;()%№@+="'
  }
...
export const validateComment = (value: string) => {
  ...
  const pattern = {
    condition: /^[\wа-яА-ЯёЁ\d.,?!:;()%№@+="\s-]+$/,
    message: 'Поле может содержать только русские/латинские буквы и символы "-.,?!-:;()%№@+="'
...
export const validateConfirmPassword = (value: string, checkAgainstValue: string) => {
  if (value === checkAgainstValue) { return '' }
...
export type TFormData = {[key: string]: {
  value: any,
  error: string,
  required: boolean
}}
  ...
  for (const key in formData) {
    if (formData[key].required) {
      if (key === 'phone') {
        ...
        formData[key].error = formData[key].value ? '' : 'Необходимо пройти антиспам проверку'
        if (formData[key].error) {
          error = true
        }
      } else if (key === 'specialist') {
        formData[key].error = formData[key].value ? '' : 'Выберите специалиста';
        if (formData[key].error) {
          error = true
        }
      } else if (key === 'comment') {
        ...
        formData[key].error = formData[key].value ? '' : 'Поле необходимо к заполнению';
        if (formData[key].error) {
          error = true
        }
      }
